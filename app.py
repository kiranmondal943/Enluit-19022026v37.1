...
# --- 1. APP CONFIGURATION ---
st.set_page_config(
    page_title="Titan v42.0 | Feature Perfect", 
    layout="wide", 
    page_icon="⚡",
    initial_sidebar_state="expanded"
)

# --- 2. ADVANCED UI SYSTEM (STREAMLIT INTERFACE - UNTOUCHED) ---
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    st.caption("v42.0 | Ultra-Modern + Full Logic")
    st.divider()
    ...
    with st.expander("🎨 Design Studio", expanded=True):
        theme_mode = st.selectbox("Base Theme", ["Clean Corporate (Light)", "Midnight Glass (Dark)", "Glassmorphism (Blur)", "Cyberpunk Neon", "Luxury Gold", "Forest Eco", "Ocean Breeze", "Stark Minimalist"])
        c1, c2 = st.columns(2)
        ...
        # LOGIC: Calculate Border Radius for CSS based on selection
        border_rad = "12px"
        ...
        h_font = st.selectbox("Headings Font", ["Outfit", "Space Grotesk", "Montserrat", "Playfair Display", "Oswald", "Clash Display"])
        b_font = st.selectbox("Body Font", ["Plus Jakarta Sans", "Inter", "Open Sans", "Roboto", "Satoshi"])
...
# --- 4. MAIN WORKSPACE ---
st.title("🏗️ StopWebRent Site Builder v42.0")
...
# ==========================================
# 4. COMPILER ENGINE (ULTRA MODERN CSS + V37 LOGIC)
# ==========================================
...
def get_theme_css():
    # Mapping v37 Themes to Modern Colors
    ...
    if "Midnight" in theme_mode or "Cyberpunk" in theme_mode:
        ...
        bg_color, text_color, card_bg = "#111111", "#D4AF37", "rgba(20, 20, 20, 0.8)"
        nav_bg, border_color = "rgba(20, 20, 20, 0.9)", "#D4AF37"
    ...
    :root {{
        ...
        --border: {border_color};
        --radius: {border_rad}; /* Using the variable calculated in Python */
    ...
    * {{ box-sizing: border-box; outline: none; }}
    ...
    nav {{ position: fixed; top: 0; width: 100%; z-index: 1000; background: var(--nav-bg); backdrop-filter: var(--glass); -webkit-backdrop-filter: var(--glass); border-bottom: 1px solid var(--border); padding: 1rem 0; transition: top 0.3s; }}
    ...
    .card:hover {{ transform: translateY(-8px); border-color: var(--p); box-shadow: var(--shadow-lg); }}
    ...
    /* SOCIAL SHARE RESTORED FROM V37 */
    ...
    /* POPUP RESTORED FROM V37 */
    #lead-popup {{ display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: var(--bg); padding: 3rem; text-align: center; border-radius: var(--radius); z-index: 2000; box-shadow: var(--shadow-lg); width: 90%; max-width: 450px; border: 1px solid var(--border); }}
...
def gen_cart_system():
    ...
    function checkoutWA() {{
        let msg = "New Order:%0A"; cart.forEach(i => {{ msg += `- ${{i.name}} (${{i.price}})%0A`; }});
        msg += "%0ATotal: " + document.getElementById('cart-total').innerText + "%0A%0A" + payLinks;
...
def gen_popup():
    ...
    return f"""<div id="lead-popup" class="modal"><div class="modal-content" style="text-align:center;"><span class="close-popup" onclick="document.getElementById('lead-popup').style.display='none'">&times;</span><h3 class="gradient-text">{popup_title}</h3><p>{popup_text}</p><a href="{top_bar_link}" class="btn btn-primary">{popup_cta}</a></div></div><script>setTimeout(() => {{ if(!localStorage.getItem('popupShown')) {{ document.getElementById('lead-popup').style.display='block'; localStorage.setItem('popupShown','true'); }} }}, {popup_delay * 1000});</script>"""
...
def gen_blog_post_html():
    # RESTORED SOCIAL SHARE FROM V37
    return f"""
        ...
        try {{
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(r[0] === slug) {{
                    ...
                    container.innerHTML = `
                        <section class="hero" style="min-height:40vh; background:var(--p);"><div class="container"><h1>${{r[1]}}</h1></div></section>
                        <div class="container" style="padding:4rem 1rem; max-width:800px;">
                            <img src="${{r[5]}}" style="width:100%; border-radius:var(--radius); margin-bottom:2rem;">
                            <div style="line-height:1.8; opacity:0.9;">${{contentHtml}}</div>
                            <div class="share-row" style="margin-top:2rem;">
                                <a href="https://wa.me/?text=${{u}}" class="share-btn bg-wa"><svg viewBox="0 0 24 24"><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91c0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21c5.46 0 9.91-4.45 9.91-9.91c0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23c-1.48 0-2.93-.39-4.19-1.15l-.3-.17l-3.12.82l.83-3.04l-.2-.32a8.188 8.188 0 0 1-1.26-4.38c.01-4.54 3.7-8.24 8.25-8.24m-3.53 3.16c-.13 0-.35.05-.54.26c-.19.2-.72.7-.72 1.72s.73 2.01.83 2.14c.1.13 1.44 2.19 3.48 3.07c.49.21.87.33 1.16.43c.49.16.94.13 1.29.08c.4-.06 1.21-.5 1.38-.98c.17-.48.17-.89.12-.98c-.05-.09-.18-.13-.37-.23c-.19-.1-.1.13-.1.13s-1.13-.56-1.32-.66c-.19-.1-.32-.15-.45.05c-.13.2-.51.65-.62.78c-.11.13-.23.15-.42.05c-.19-.1-.8-.3-1.53-.94c-.57-.5-1.02-1.12-1.21-1.45c-.11-.19-.01-.29.09-.38c.09-.08.19-.23.29-.34c.1-.11.13-.19.19-.32c.06-.13.03-.24-.01-.34c-.05-.1-.45-1.08-.62-1.48c-.16-.4-.36-.34-.51-.35c-.11-.01-.25-.01-.4-.01Z"/></path></svg></a>
                                <a href="https://www.facebook.com/sharer/sharer.php?u=${{u}}" class="share-btn bg-fb"><svg viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg></a>
                            </div>
                            <a href="blog.html" class="btn btn-outline" style="margin-top:2rem;">&larr; Back to Blog</a>
                        </div>`;
                    break;
                }}
            }}
...
def gen_product_page_content(is_demo=False):
    ...
    function changeMainImg(src) {{ 
        ...
        setTimeout(() => {{ document.getElementById('main-img').src = src; document.getElementById('main-img').style.opacity = 1; }}, 200);
    }}
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(clean[0] === targetName) {{
                    ...
                    if(imgs.length > 1) {{
                        ...
                        imgs.forEach(url => {{ thumbs += `<img src="${{url}}" style="width:100%; aspect-ratio:1; object-fit:cover; border-radius:8px; cursor:pointer;" onclick="changeMainImg('${{url}}')">`; }});
                        thumbs += '</div>';
                    }}
                    
                    let mainBtn = stripeLink ? `<a href="${{stripeLink}}" class="btn btn-primary" style="flex:1;">Buy Now</a>` : `<button onclick="addToCart('${{clean[0].replace(/'/g,"\\'")}}', '${{clean[1].replace(/'/g,"\\'")}}')" class="btn btn-primary" style="flex:1;">Add to Cart</button>`;
                    const u = encodeURIComponent(window.location.href);

                    document.getElementById('product-detail').innerHTML = `
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:4rem; align-items:start;">
                            <div><img id="main-img" src="${{mainImg}}" style="width:100%; border-radius:24px; box-shadow:var(--shadow-lg); transition:opacity 0.2s;">${{thumbs}}</div>
                            <div>
                                ...
                                <div style="display:flex; gap:1rem;">${{mainBtn}}</div>
                                <div class="share-row" style="margin-top:2rem;">
                                    <a href="https://wa.me/?text=${{u}}" class="share-btn bg-wa"><svg viewBox="0 0 24 24"><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91c0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21c5.46 0 9.91-4.45 9.91-9.91c0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23c-1.48 0-2.93-.39-4.19-1.15l-.3-.17l-3.12.82l.83-3.04l-.2-.32a8.188 8.188 0 0 1-1.26-4.38c.01-4.54 3.7-8.24 8.25-8.24m-3.53 3.16c-.13 0-.35.05-.54.26c-.19.2-.72.7-.72 1.72s.73 2.01.83 2.14c.1.13 1.44 2.19 3.48 3.07c.49.21.87.33 1.16.43c.49.16.94.13 1.29.08c.4-.06 1.21-.5 1.38-.98c.17-.48.17-.89.12-.98c-.05-.09-.18-.13-.37-.23c-.19-.1-.1.13-.1.13s-1.13-.56-1.32-.66c-.19-.1-.32-.15-.45.05c-.13.2-.51.65-.62.78c-.11.13-.23.15-.42.05c-.19-.1-.8-.3-1.53-.94c-.57-.5-1.02-1.12-1.21-1.45c-.11-.19-.01-.29.09-.38c.09-.08.19-.23.29-.34c.1-.11.13-.19.19-.32c.06-.13.03-.24-.01-.34c-.05-.1-.45-1.08-.62-1.48c-.16-.4-.36-.34-.51-.35c-.11-.01-.25-.01-.4-.01Z"/></path></svg></a>
                                </div>
                            </div>
...
st.download_button(
    label="📥 DOWNLOAD ULTRA-MODERN WEBSITE",
    data=z_b.getvalue(),
    file_name="titan_ultra_modern_site.zip",
