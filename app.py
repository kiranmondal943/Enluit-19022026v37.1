...
with st.sidebar:
    ...
    with st.expander("🎨 Design Studio", expanded=True):
        theme_mode = st.selectbox("Base Theme", ["Modern Glass (Light)", "Midnight Glass (Dark)", "Neo-Brutalism", "Luxury Gold"])
        c1, c2 = st.columns(2)
        p_color = c1.color_picker("Primary Brand", "#2563EB") 
        s_color = c2.color_picker("Accent (CTA)", "#F43F5E")  
        
        st.markdown("**Layout**")
        hero_layout = st.selectbox("Hero Alignment", ["Left", "Center"])
        
        # Font logic handled in CSS generator
...
with tabs[2]:
    ...
    top_bar_text = st.text_input("Promo Text", "🔥 50% OFF Launch Sale - Limited Time")
...
with tabs[4]:
    st.subheader("🛒 Store")
    st.info("Multi-Image: Separate URLs with `|` (e.g. `img1.jpg|img2.jpg`)")
...
def get_theme_css():
    ...
    * {{ box-sizing: border-box; outline: none; }}
    ...
    .btn-sm {{ padding: 0.6rem 1rem; font-size: 0.85rem; border-radius: 8px; }}
...
def gen_nav():
    ...
    return f"""
    {f'<div id="top-bar" style="background:var(--s); color:white; text-align:center; padding:8px; font-size:0.85rem; font-weight:600;"><a href="{top_bar_link}" style="color:white; text-decoration:underline;">{top_bar_text}</a></div>' if top_bar_enabled else ''}
    <nav>
    ...
    <div id="langModal" class="modal">
        <div class="modal-content">
            ...
            <div style="display:grid; gap:10px; margin-top:20px;">
                <button onclick="switchLanguage('en')" class="btn btn-outline" style="width:100%; justify-content:flex-start;">🇺🇸 English</button>
                <button onclick="switchLanguage('es')" class="btn btn-outline" style="width:100%; justify-content:flex-start;">🇪🇸 Español / Other</button>
            </div>
        </div>
    </div>
    <script>
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(clean[0] === targetName) {{
                    ...
                    if(window.innerWidth < 768) {{
                         document.getElementById('product-detail').children[0].style.gridTemplateColumns = '1fr';
                    }}
                    break;
                }}
            }}
...
def gen_cart_system():
    ...
    <div id="cartModal" class="modal">
        <div class="modal-content">
            ...
            <div id="cart-items" style="max-height: 300px; overflow-y: auto; margin: 20px 0;">
                <p style="opacity:0.6; text-align:center;">Your cart is empty.</p>
            </div>
            ...
            <button class="btn btn-primary" style="width: 100%; margin-top: 20px;" onclick="checkoutWA()">
    ...
    function updateCart() {{
        ...
        if (cart.length === 0) {{
            box.innerHTML = '<p style="opacity:0.6; text-align:center;">Your cart is empty.</p>';
        ...
        cart.forEach((item, index) => {{
            html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid var(--border);">
                <div>
                    ...
                    <div style="font-size:0.9rem; opacity:0.8;">${{item.price}}</div>
                </div>
                <div onclick="removeFromCart(${{index}})" style="color:var(--s); cursor:pointer; padding:5px;">✕</div>
            </div>`;
...
# --- PLACEHOLDERS FOR UNDEFINED FUNCTIONS FROM ORIGINAL SNIPPET ---
# (Re-injecting necessary parts from the previous stable code to ensure full execution)
def gen_popup():
    if not popup_enabled: return ""
    return f"""<div id="lead-popup" class="modal"><div class="modal-content" style="text-align:center;">
        <span class="close-modal" onclick="closeModal('lead-popup')" style="position:absolute; right:15px; top:15px; cursor:pointer;">&times;</span>
        <h3 class="gradient-text">{popup_title}</h3><p>{popup_text}</p><a href="{top_bar_link}" class="btn btn-primary">{popup_cta}</a>
    </div></div>
    <script>setTimeout(() => {{ if(!sessionStorage.getItem('pop')) {{ document.getElementById('lead-popup').style.display='block'; sessionStorage.setItem('pop','1'); }} }}, {popup_delay}000);</script>"""
def gen_lang_script():
    return "" # Logic handled in nav now for simplicity or can be re-added
def gen_stats():
...
preview_mode = st.radio("Preview Page:", ["Home", "Product Detail (Demo)"], horizontal=True)
...
st.success("System Ready. Click below to download your Ultra-Modern Site.")
z_b = io.BytesIO()
